...
export class EgoBrowserUnavailableError extends Error {
    ...
    constructor() {
        super('ego-browser is required for public Explorer queries. Install ego lite from https://lite.ego.app/ and complete onboarding.');
 ...
 * partial evidence.
 */
export function createBrowserChainAnalysisClient(options) {
    ...
    return {
        ...
        async getTransaction(input, requestOptions = {}) {
            ...
            const load = browserQueue.then(() => loadBrowserTransaction({
                reference,
                pageEvaluator,
                timeoutMs,
            ...
            browserQueue = load.then(() => undefined, () => undefined);
            return await load;
        },
    };
}
...
export async function resolveBrowserChromeExecutable(configured) {
    const candidates = [
        configured?.trim(),
...
export async function resolveEgoBrowserExecutable(pathValue = process.env.PATH) {
...
export function createEgoBrowserPageEvaluator(options = {}) {
...
async function loadBlockscoutTransaction(input) {
    ...
    if (!isRecord(raw))
        throw new Error('Blockscout browser page returned invalid JSON.');
    ...
    const parsed = browserEvmTransactionSchema.parse({
        ...
        status: raw.status === 'ok' || raw.result === 'success'
            ? 'success'
...
export function createScanPageTransactionExpression() {
    return `(() => {
...
export function resolveSolanaBrowserTransactionId(reference, network) {
    ...
    try {
        ...
        if (match?.[1] === undefined)
            throw new Error('Expected a Solscan transaction URL.');
...
export async function prepareBrowserProfile(profileDirectory) {
    ...
    await Promise.all(['SingletonLock', 'SingletonCookie', 'SingletonSocket', 'DevToolsActivePort'].map(async (name) => {
        try {
...
export async function resolveExplorerChromeLaunch(chromeExecutable, chromeArguments, options = {}) {
...
function normalizeSolscanPayload(payload) {
    ...
    return {
        accountKeys: native.flatMap((item) => isRecord(item) && typeof item.address === 'string' ? [item.address] : []),
        ...
        executionStatus: data.status === 'Fail' || data.status === 'failed'
            ? 'reverted'
        ...
        nativeChanges: native.flatMap((item) => isRecord(item) &&
...
function collectProgramIds(value, output) {
    ...
    for (const item of value) {
        if (!isRecord(item))
            continue;
        if (typeof item.program_id === 'string')
            output.push(item.program_id);
...
//# sourceMappingURL=browser-chain-analysis-client.js.map