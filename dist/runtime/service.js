import { assessXxyyPoolSelection, assessXxyySandwichPattern, xxyyPoolPolicySchema, } from '../xxyy-diagnosis-core/index.js';
import { diagnoseXxyyTransactionInputSchema, diagnoseXxyyTransactionOutputSchema, } from './contracts.js';
export function createXxyyTransactionDiagnosisService(options) {
    ...
    return {
        async diagnoseXxyyTransaction(rawInput, requestOptions = {}) {
            ...
            if ((transaction.family === 'solana' &&
                ...
                warnings.push('Transaction facts came from a fixed Explorer browser page and are partial, single-source evidence.');
            }
            const market = context.targetTokenAddresses.length === 0
                ? undefined
            ...
            const surroundingTrades = input.checks.includes('sandwich') && market?.status === 'exact'
                ? await resolveSurroundingTrades(market, context.chain, options.chainAnalysis, requestOptions.signal)
                : [];
            ...
            if (input.checks.includes('sandwich')) {
                if (market?.status === 'exact') {
                    ...
                    warnings.push('Browser and XXYY rows can support a same-block/slot structural pattern, but pool-state and profit/loss evidence remain unavailable for confirmation.');
                }
                else {
...
function extractLookupContext(transaction) {
    if (transaction.family === 'evm') {
        ...
        const tokenAddresses = unique([
            ...
            ...transaction.analysis.assetChanges.flatMap((change) => change.asset.kind === 'erc20' ? [change.asset.contractAddress] : []),
        ...
        return {
            ...
            ...(timestamp === undefined ? {} : { timestampMs: Number(BigInt(timestamp) * 1000n) }),
        };
    }
...
function diagnosisStatus(input) {
    ...
    const conclusionIncomplete = input.sandwichAssessment?.verdict === 'insufficient_data' ||
...
function assessMarketSandwichEvidence(transaction, market, surroundingTrades) {
    ...
    const neighborhoodComplete = earlier !== undefined &&
        later !== undefined &&
...
async function resolveSurroundingTrades(market, network, chainAnalysis, signal) {
    return await Promise.all((market.contextTrades ?? []).map(async (trade) => {
        if (network.startsWith('eip155:') && trade.blockNumber !== undefined) {
            return { ...trade, chainStatus: 'resolved' };
        }
        try {
            const transaction = await chainAnalysis.getTransaction({ network, reference: trade.transactionId }, signal === undefined ? {} : { signal });
            if (transaction.family === 'evm') {
...
function buildSummary(transaction, marketStatus, screenshotReady) {
    const match = marketStatus === 'exact'
