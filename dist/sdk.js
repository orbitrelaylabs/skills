...
import { createBrowserChainAnalysisClient, createEgoBrowserPageEvaluator, EgoBrowserUnavailableError, resolveBrowserChromeExecutable, resolveEgoBrowserExecutable, } from './runtime/browser-chain-analysis-client.js';
...
import { diagnoseXxyyTransactionInputSchema, } from './runtime/contracts.js';
import { getTransactionInputSchema, } from './runtime/public-transaction-contracts.js';
...
export async function inspectPublicTransaction(input, options = {}) {
    ...
    try {
        return await client.getTransaction(parsedInput, options.signal === undefined ? {} : { signal: options.signal });
    }
    finally {
        await client.close();
    }
}
export async function diagnoseXxyyTransaction(input, options = {}) {
    ...
    try {
        const canonicalPoolConfig = options.canonicalPoolConfig ?? env.XXYY_CANONICAL_POOL_CONFIG_JSON?.trim();
        const screenshotProvider = captureScreenshot
            ? await createScreenshotProvider({
                ...
                ...((options.chromeExecutable ?? env.XXYY_SCREENSHOT_CHROME_EXECUTABLE?.trim())
                    ? {
                        chromeExecutable: options.chromeExecutable ?? env.XXYY_SCREENSHOT_CHROME_EXECUTABLE.trim(),
                    }
                    : {}),
            })
            : undefined;
        const service = createXxyyTransactionDiagnosisService({
            ...
            poolPolicy: {
                maxSmallPoolLiquidityUsd: options.maxSmallPoolLiquidityUsd ??
                    env.XXYY_SMALL_POOL_MAX_LIQUIDITY_USD?.trim() ??
                    '10000',
                maxSmallPoolRelativeLiquidityPpm: options.maxSmallPoolRelativeLiquidityPpm ??
        ...
        const output = await service.diagnoseXxyyTransaction(parsedInput, options.signal === undefined ? {} : { signal: options.signal });
        const screenshotFilePath = output.screenshotEvidence.status === 'ready'
...
async function requireEgoBrowser(options) {
