...
import { createBrowserChainAnalysisClient, createEgoBrowserPageEvaluator, EgoBrowserUnavailableError, resolveBrowserChromeExecutable, resolveEgoBrowserExecutable, } from './browser-chain-analysis-client.js';
...
import { XXYY_TRANSACTION_DIAGNOSIS_RUNTIME_VERSION, diagnoseXxyyTransactionInputSchema, } from './contracts.js';
import { createXxyyTransactionDiagnosisService } from './service.js';
export async function runXxyyTransactionDiagnosisCli(options = {}) {
    ...
    const chromeExecutable = await resolveBrowserChromeExecutable(env.XXYY_SCREENSHOT_CHROME_EXECUTABLE);
    ...
    const profileDirectory = path.resolve(env.XXYY_BROWSER_PROFILE_DIRECTORY?.trim() || path.join(stateDirectory, 'browser-profile'));
    const artifactDirectory = path.resolve(parsed.outputDirectory ??
    ...
    try {
        ...
        const service = createXxyyTransactionDiagnosisService({
            ...
            poolPolicy: {
                ...
                maxSmallPoolRelativeLiquidityPpm: parseRelativeLiquidityPpm(env.XXYY_SMALL_POOL_MAX_RELATIVE_LIQUIDITY_PPM),
                version: '1.0.0',
            },
        ...
        const output = await service.diagnoseXxyyTransaction(diagnoseXxyyTransactionInputSchema.parse({
...
function parseCliArguments(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (!argument.startsWith('--'))
            throw new TypeError(`Unknown positional argument: ${argument}`);
