...
import { xxyyScreenshotArtifactSchema } from './contracts.js';
import { prepareBrowserProfile, resolveExplorerChromeLaunch, } from './browser-chain-analysis-client.js';
const DEFAULT_CAPTURE_TIMEOUT_MS = 60_000;
const MAX_SCREENSHOT_BYTES = 10 * 1_048_576;
export function createChromeXxyyScreenshotProvider(options) {
    ...
    const persistentProfileDirectory = options.profileDirectory === undefined
        ? undefined
...
async function prepareNativeEvidencePage(input) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        const filterWindows = input.input.timestamp === undefined ? [undefined] : [5_000, 15_000, 120_000];
        ...
        if (!highlighted) {
            throw new Error('The exact trade was verified by API but its native XXYY trade row was not rendered.');
        }
        if (!klineReady ||
...
async function terminateChrome(process) {
...
function signalBrowserProcess(process, signal) {
...
export function xxyyPairUrl(chain, pairAddress) {
    const slug = {
        'eip155:1': 'eth',
        'eip155:56': 'bsc',
        'eip155:988': 'stable',
...
export async function readDebuggerUrl(process, timeoutMs, signal) {
    const stderr = process.stderr;
    if (stderr === null)
        throw new Error('Chrome stderr was unavailable.');
    return await new Promise((resolve, reject) => {
        let buffer = '';
        const timeout = setTimeout(() => finish(new Error('Chrome debugger startup timed out.')), timeoutMs);
        ...
        const data = (chunk) => {
            ...
            if (match?.[1] !== undefined)
                finish(undefined, new URL(match[1]));
        };
        const finish = (error, value) => {
            ...
            if (error === undefined && value !== undefined)
                ...
                reject(error ?? new Error('Chrome debugger URL was missing.'));
        };
        stderr.on('data', data);
...
export async function findPageDebuggerUrl(debuggerUrl, timeoutMs, signal) {
    const endpoint = new URL('/json/list', `http://${debuggerUrl.host}`);
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        if (signal?.aborted === true)
            throw new Error('XXYY screenshot capture was aborted.');
        try {
...
export async function createCdpClient(url, timeoutMs, signal) {
    ...
    socket.addEventListener('message', (event) => {
        ...
        if (isRecord(message.error))
            request.reject(new Error('CDP command failed.'));
        else
...
async function pollForVerifiedRow(input) {
    ...
    const amountFragments = [input.tokenAmount, input.nativeAmount, input.usdAmount].flatMap(compactAmountFragments);
...
export function buildKlineReadinessExpression() {
    return `(() => {
...
export function buildNativeHistoricalFilterExpression(timestamp, windowMs = 120_000) {
...
function recordString(value, key) {
    ...
    if (typeof item !== 'string')
        throw new Error(`CDP response omitted ${key}.`);
    return item;
}
...
//# sourceMappingURL=chrome-screenshot-provider.js.map