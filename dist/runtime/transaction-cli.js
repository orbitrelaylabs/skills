...
import { createBrowserChainAnalysisClient, createEgoBrowserPageEvaluator, EgoBrowserUnavailableError, resolveEgoBrowserExecutable, } from './browser-chain-analysis-client.js';
import { getTransactionInputSchema } from './public-transaction-contracts.js';
export async function runPublicTransactionCli(options = {}) {
    ...
    try {
        const output = await client.getTransaction(getTransactionInputSchema.parse({
...
export async function publicTransactionMain(options = {}) {
    const argv = options.argv ?? process.argv.slice(2);
    ...
    try {
        ...
        process.stdout.write(`${JSON.stringify(output, null, argv.includes('--pretty') ? 2 : undefined)}\n`);
    }
    catch (error) {
...
//# sourceMappingURL=transaction-cli.js.map