import { ERC20_TRANSFER_TOPIC, EVM_ZERO_ADDRESS, TRANSACTION_ANALYSIS_SKILL, TRANSACTION_ANALYSIS_VERSION, evmTransactionSnapshotSchema, transactionAnalysisResultSchema, } from './contracts.js';
const findingIds = {
...
export function analyzeEvmTransactionSnapshot(input) {
    ...
    const feeWei = receipt === undefined ? undefined : BigInt(receipt.gasUsed) * BigInt(receipt.effectiveGasPrice);
    const nativeRecipient = transaction.to ?? receipt?.contractAddress ?? undefined;
    const hasConfirmedNativeTransfer = executionStatus === 'success' && valueWei > 0n && nativeRecipient !== undefined;
    ...
    const blockEvidenceId = block === undefined ? undefined : `block:${snapshot.chainId}:${block.number}`;
    const feeEvidenceId = feeWei === undefined ? undefined : `calculation:fee:${transaction.hash}`;
    const transactionSupports = [
    ...
    state.evidence.push(createEvidence(snapshot, sources, {
    ...
    if (receipt !== undefined && receiptEvidenceId !== undefined) {
        const receiptSupports = [findingIds.executionStatus, findingIds.gasFee];
    ...
    if (hasConfirmedNativeTransfer &&
        ...
        addAssetDelta(state, transaction.from, { chainId: snapshot.chainId, kind: 'native' }, -valueWei, evidenceIds);
        addAssetDelta(state, nativeRecipient, { chainId: snapshot.chainId, kind: 'native' }, valueWei, evidenceIds);
    }
    if (receipt !== undefined &&
        receiptEvidenceId !== undefined &&
        feeWei !== undefined &&
        ...
        addAssetDelta(state, transaction.from, { chainId: snapshot.chainId, kind: 'native' }, -feeWei, evidenceIds);
    }
    if (decodedTransfers.length > 0 && receipt !== undefined && receiptEvidenceId !== undefined) {
        ...
        for (const transfer of decodedTransfers) {
            ...
            if (transfer.from !== EVM_ZERO_ADDRESS) {
                addAssetDelta(state, transfer.from, { contractAddress: transfer.tokenAddress, kind: 'erc20' }, -amount, [transfer.evidenceId]);
            }
            if (transfer.to !== EVM_ZERO_ADDRESS) {
                addAssetDelta(state, transfer.to, { contractAddress: transfer.tokenAddress, kind: 'erc20' }, amount, [transfer.evidenceId]);
            }
        }
    }
    ...
    const status = state.materialIssue ? 'partial' : 'success';
    return finalizeResult(snapshot, state, {
...
function addConflictArtifacts(snapshot, state) {
    ...
    for (const [index, conflict] of conflicts.entries()) {
        ...
        const sourceIds = Array.from(new Set(conflict.observations.map((observation) => observation.sourceId))).sort();
...
function validateSourceReferences(snapshot, sources, state) {
...
function decodeErc20Transfers(receipt, transactionHash, state) {
    if (receipt === undefined) {
        return [];
    }
    ...
    for (const log of [...receipt.logs].sort((left, right) => left.logIndex - right.logIndex)) {
        ...
        transfers.push({
            ...
            transferType: from === EVM_ZERO_ADDRESS ? 'mint' : to === EVM_ZERO_ADDRESS ? 'burn' : 'transfer',
        });
    }
    return transfers;
}
function isAddressTopic(topic) {
    return /^0x0{24}[0-9a-f]{40}$/u.test(topic);
}
function topicToAddress(topic) {
    return `0x${topic.slice(-40)}`;
}
function classifyInput(input, to) {
    if (to === null) {
        return 'contract_creation';
    }
    return input === '0x' ? 'native_transfer' : 'contract_call';
}
function executionStatement(status) {
    if (status === 'success') {
...
function createSummary(executionStatus, transferCount, status) {
    const execution = executionStatus === 'success'
...
function createEvidence(snapshot, sources, input) {
...
function addAssetDelta(state, address, asset, delta, evidenceIds) {
    if (delta === 0n) {
        return;
    }
    const assetId = asset.kind === 'native' ? `native:${asset.chainId}` : `erc20:${asset.contractAddress}`;
...
function finalizeResult(snapshot, state, input) {
...
function addDiagnostic(state, stage, code, retryable, evidenceIds) {
...
//# sourceMappingURL=analyze-transaction.js.map