...
export const TRANSACTION_ANALYSIS_SKILL = 'transaction_analysis';
export const TRANSACTION_ANALYSIS_VERSION = '1.0.0';
...
export const evmChainIdSchema = z
    ...
    .refine((value) => positiveUintPattern.test(value) && BigInt(value) <= EVM_UINT256_MAX, 'EVM chain id exceeds uint256.');
export const evmUintSchema = z
    .string()
    .trim()
    .max(78)
    .regex(canonicalUintPattern, 'Expected an unsigned decimal integer.')
    .refine((value) => canonicalUintPattern.test(value) && BigInt(value) <= EVM_UINT256_MAX, 'Unsigned integer exceeds uint256.');
...
const receiptSchema = z
    ...
    .strict()
    .refine((receipt) => !isUint256(receipt.gasUsed) ||
...
const sourceConflictSchema = z
    ...
    observations: z
        ...
        .min(2)
        .max(8)
        .refine((observations) => new Set(observations.map((observation) => observation.sourceId)).size >= 2, 'Conflicts require at least two distinct sources.')
        .refine((observations) => new Set(observations.map((observation) => observation.value)).size >= 2, 'Conflicts require at least two distinct values.'),
})
    .strict();
export const evmTransactionSnapshotSchema = z
    ...
    .strict();
export const transactionExecutionStatuses = ['success', 'reverted', 'pending', 'unknown'];
