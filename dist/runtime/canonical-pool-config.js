...
export function createConfiguredCanonicalPoolResolver(rawConfig) {
    const config = xxyyCanonicalPoolConfigSchema.parse(typeof rawConfig === 'string' ? parseJson(rawConfig) : rawConfig);
    const byToken = new Map(config.entries.map((entry) => [
        `${entry.chain.toLowerCase()}:${normalizeIdentifier(entry.chain, entry.tokenAddress)}`,
        entry.pairAddress,
    ]));
    return (input) => {
        const matches = new Set(input.targetTokenAddresses.flatMap((token) => {
            const pair = byToken.get(`${input.chain.toLowerCase()}:${normalizeIdentifier(input.chain, token)}`);
...
//# sourceMappingURL=canonical-pool-config.js.map