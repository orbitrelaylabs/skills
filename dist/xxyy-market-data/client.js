...
import { XXYY_MARKET_DATA_ORIGIN, xxyyMarketTradeSchema, xxyyTradeLookupInputSchema, xxyyTradeLookupResultSchema, } from './contracts.js';
import { XxyyMarketDataError } from './errors.js';
const DEFAULT_REQUEST_TIMEOUT_MS = 15_000;
const DEFAULT_MAX_RESPONSE_BYTES = 1_048_576;
const TRADE_WINDOW_MS = 120_000;
const TRADE_SEARCH_WINDOWS_MS = [2_000, 15_000, TRADE_WINDOW_MS];
...
const tradeSearchResponseSchema = z
    ...
    .passthrough();
export function createXxyyMarketDataClient(options = {}) {
    ...
    return {
        async findTrade(rawInput, requestOptions = {}) {
            ...
            return xxyyTradeLookupResultSchema.parse({
                candidatePairs,
                contextTrades: contextTradesForMatch(match.trade, tradeSearch.tradesByPair.get(match.pair.pairAddress) ?? []),
                diagnostics,
...
async function loadCandidatePairs(input) {
    ...
    for (const tokenAddress of input.input.targetTokenAddresses) {
        if (pairs.size >= MAX_PAIR_CANDIDATES)
            break;
        try {
            const response = pairSearchResponseSchema.parse(await requestJson({
                fetchImpl: input.fetchImpl,
                headers: xxyyHeaders(input.input.chain),
                maxResponseBytes: input.maxResponseBytes,
                requestTimeoutMs: input.requestTimeoutMs,
                ...(input.signal === undefined ? {} : { signal: input.signal }),
                url: new URL(`/api/data/search/v3?q=${encodeURIComponent(tokenAddress)}`, XXYY_MARKET_DATA_ORIGIN),
            }));
    ...
    return [...pairs.values()].sort((left, right) => left.pairAddress.localeCompare(right.pairAddress));
}
    ...
    for (const pair of input.candidatePairs) {
        try {
            ...
            for (const windowMs of windows) {
                const response = tradeSearchResponseSchema.parse(await requestJson({
                ...
                if (responseTrades.some((trade) => identifierEquals(input.input.chain, trade.transactionId, input.input.transactionId)) &&
...
function tradeDistance(target, candidate) {
    if (target.blockNumber !== undefined && candidate.blockNumber !== undefined) {
        ...
        const blockDistance = Number(absolute > BigInt(Number.MAX_SAFE_INTEGER) ? BigInt(Number.MAX_SAFE_INTEGER) : absolute);
...
function tradeRelation(target, candidate) {
    if (target.blockNumber !== undefined && candidate.blockNumber !== undefined) {
        ...
        if (target.logIndex !== undefined && candidate.logIndex !== undefined) {
            if (candidate.logIndex < target.logIndex)
                return 'earlier';
            if (candidate.logIndex > target.logIndex)
                return 'later';
        }
...
function disambiguateMatchesByTransactionAccounts(input, matches) {
    if (matches.length <= 1 || input.transactionAccountAddresses === undefined)
        return matches;
    const observed = new Set(input.transactionAccountAddresses.map((address) => normalizeIdentifier(input.chain, address)));
    const onchainMatches = matches.filter((match) => observed.has(normalizeIdentifier(input.chain, match.pair.pairAddress)));
    return onchainMatches.length === 1 ? onchainMatches : matches;
}
function candidatePairsForTradeSearch(input, candidatePairs) {
    if (input.transactionAccountAddresses === undefined)
        return candidatePairs;
    const observed = new Set(input.transactionAccountAddresses.map((address) => normalizeIdentifier(input.chain, address)));
    const observedPairs = candidatePairs.filter((pair) => observed.has(normalizeIdentifier(input.chain, pair.pairAddress)));
...
function tradeSearchBody(pairAddress, timestamp, windowMs) {
    return {
        ...
        timeStart: timestamp === undefined || windowMs === undefined ? '' : Math.max(0, timestamp - windowMs),
        tokenAmountEnd: '',
        tokenAmountStart: '',
        type: 'all',
        usdAmountEnd: '',
...
function diagnosticFor(error, stage) {
    if (error instanceof XxyyMarketDataError) {
...
export function createXxyyMarketDataClientStub(findTrade) {
    return { findTrade };
}
export function emptyXxyyTradeLookupResult() {
    return xxyyTradeLookupResultSchema.parse({
        candidatePairs: [],
        diagnostics: [],
        status: 'not_found',
